...
const getEvents = async (number: number) => {
    ...
    const res = await fetch(`https://www.googleapis.com/calendar/v3/calendars/as.tiedottaja@gmail.com/events?key=AIzaSyCJrtmGOeEFAq912lwijvCmKR33SAtC_qo&singleEvents=true&orderBy=starttime&timeMin=${startDate}&timeMax=${endDate}`)
...
const parseEventData = (data: any) => {
    ...
    const filteredEventData = data.items.filter(item => item.organizer.self).slice(0, 5);
...
const dateOptions: Intl.DateTimeFormatOptions = { timeZone: "Europe/Helsinki", weekday: "short", year: "numeric", month: "numeric", day: "numeric" };
