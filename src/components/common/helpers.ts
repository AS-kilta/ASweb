export function compareUrl(urlarr1: string[], urlarr2: string[]): number {
  let count: number = 0;
  const maxlen: number = urlarr1.length > urlarr2.length ? urlarr2.length : urlarr1.length;
  const isSameLength: boolean = urlarr1.length === urlarr2.length;
