...
import { BsDash, BsList, BsPlus, BsX } from "react-icons/bs"
...
const Subnavi: React.FC<SubnaviProps> = ({ lang, data }) => {
  ...
  const subnaviClasses = `${style.subnavi} ${
    expanded ? style.showDropdown : ""
  }`
...
const NavCollapse: React.FC<NavCollapseProps> = (
  { lang, slug, translation, isExpanded },
) => {
...
const Navbar: React.FC<NavbarProps> = ({ lang, slug, translation }) => {
  ...
  return (
    <nav
      id={style.navbarTop}
      className={navExpanded ? style.expanded : ""}
