...
const Committee: React.FC<
  { committee: CommitteeData; lang: string; year: string }
> = ({ committee, lang, year }) => {
  return (
    <div>
        ...
        <p>
          {committee.members.map((member: CommitteeMember) => (
            <Fragment key={year + member.name}>
              <strong>
                {member.title.map((title: TranslatedEntry) =>
                  title[lang]
                ).join(
                  ", ",
                )}
...
const Board: React.FC<{ board: BoardMember[]; lang: string; year: string }> = (
  { board, lang, year },
) => {
  return (
    <Fragment>
      ...
      <p>
        {board.map((member: BoardMember) => (
          <Fragment key={year + member.name}>
            <strong>{member.title[lang]}</strong> {member.name}
            <br />
          </Fragment>
        ))}
      </p>
...
const Officials: React.FC<
  { officials: CommitteeData[]; lang: string; year: string }
> = ({ officials, lang, year }) => {
  return (
    <Fragment>
...
const Accolades: React.FC<{ accolades: Accolades; lang: string }> = (
  { accolades, lang },
) => {
  return (
    <Fragment>
      ...
      {accolades.people.map((accolade: Accolade) => {
        if (!accolade.description[lang]) {
          return (
            <div key={accolade.name} className={style.noDescription}>
              {accolade.name}
            </div>
          )
        }
    ...
    <div className={style.container}>
      ...
      {data.map((entry: ArchiveEntry) => {
        if (entry.year && (entry.board || entry.officials)) {
          return (
            <CollapseBox
              key={`officials-${entry.year}`}
              title={entry.year}
              expand={allExpanded}
            >
              {entry.board && (
                <Board board={entry.board} lang={lang} year={entry.year} />
              )}
              {entry.officials && (
                <Officials
      ...
      {data.map((entry: ArchiveEntry) => {
        return entry.year && entry.accolades && (
          <CollapseBox
            key={`accolades-${entry.year}`}
            title={entry.year}
            expand={allExpanded}
          >
            {entry.accolades.map((accolades: Accolades) => (
              <Accolades
                key={`${accolades.name[lang]}`}
                accolades={accolades}
                lang={lang}
              />
            ))}
